...
const Process = () => {
    ...
    const [filteredItems, setFilteredItems] = useState([]); // State for location-specific items
    ...
    // Effect to filter items whenever master list or location changes
    useEffect(() => {
        if (masterItems.length > 0 && vendorLocation) {
            console.log(`Filtering for location: "${vendorLocation}"`);
            const filtered = masterItems.filter(
                item => item.location.toLowerCase() === vendorLocation.toLowerCase()
            );
            console.log(`Found ${filtered.length} items for this location.`);
    ...
    const handleSubmitBill = async () => {
        ...
        try {
            const billData = {
                ...
                billItems: billItems.map(({ ...item }) => item), // Clean up bill items
                totalBill,
        ...
        <div className="min-h-screen bg-gray-50 p-4 md:p-8">
            <div className="max-w-2xl mx-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        readOnly // Rate is now always set by selection
                                        disabled={billCalculated}
                                    />
                                </div>
                                <div>
                ...
                {!billCalculated ? (
                    ...
                    <div className="mt-6 flex gap-4">
                        <button onClick={() => setBillCalculated(false)} disabled={loading} className="w-full py-3 bg-gray-300 text-gray-800 font-bold rounded-lg hover:bg-gray-400">Edit</button>
